...
def svm_solver(x_train, y_train, lr, num_iters,
    ...
    for iter in range(num_iters):
        #prediction = x_train @ 
        for j in range(d):
            alphacpy = alpha.clone()
            alphacpy_p = torch.tensor(alphacpy, requires_grad=True)
            print(x_train[:,j].size())
            print(alpha.size())
            y = kernel(x_train[:,j], alphacpy_p)
            print(y)
            y.backward()
            alphacpy -= lr*y.grad
            alpha = PIc(alphacpy)
...
svm_solver(X, Y, 0.1, 100, kernel=hw2_utils.poly(degree=1), c=0)